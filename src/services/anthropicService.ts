...
export class AnthropicService implements AiService {
  ...
  constructor(apiKey: string, model: string) {
    ...
    this.model = model;
    this.promptTemplate = PromptUtils.loadPromptTemplate();
  }
  ...
  private getModelCosts(model: string): { input: number; output: number } {
    ...
    if (!costs) {
      throw new Error(
        `Invalid model: ${model}. Please select a valid model in settings.`
      );
    }
  ...
  async analyze(text: string): Promise<AnalysisResponse> {
    ...
    const system = prompt.match(
      /<PROMPT_LINTER_SYSTEM_PROMPT>([\s\S]*?)<\/PROMPT_LINTER_SYSTEM_PROMPT>/
    )?.[1];
    const user = prompt.match(
      /<PROMPT_LINTER_USER_PROMPT>([\s\S]*?)<\/PROMPT_LINTER_USER_PROMPT>/
    )?.[1];

    if (!system || !user) {
      throw new Error(
        "The analysis prompt (src/prompts/analyze.prompt) must have a <PROMPT_LINTER_SYSTEM_PROMPT> and <PROMPT_LINTER_USER_PROMPT> section."
      );
    }
  ...
  private createMetadata(response: any) {
    ...
    const inputCost =
      (response.usage.input_tokens * modelCosts.input) / 1000000;
    const outputCost =
      (response.usage.output_tokens * modelCosts.output) / 1000000;
    ...
    return {
      ...
      cost: `$${totalCost} (Input: $${inputCost.toFixed(
        6
